 ...
 * Seed data.
 * ----------
 * The simulation ships with a realistic but entirely fictional schedule.
 * Because there is no database, the schedule is generated in the browser the
 * first time the application runs and then persisted to localStorage, after
 * which the administrator can add, edit or delete flights like any other
 * record.
 */

import type { Airport, CabinConfig, Flight } from "./types";
...
/** Number of days of schedule generated ahead of the current date. */
...
/** Format a Date as a local-time ISO-like string, avoiding UTC drift. */
...
/**
 * Expand the route templates into concrete flight instances covering
 * `horizonDays` starting from `from` (defaults to today).
 */
export function generateSchedule(from: Date = new Date(), horizonDays: number = SCHEDULE_HORIZON_DAYS): Flight[] {
  ...
  for (let dayOffset = 0; dayOffset < horizonDays; dayOffset += 1) {
    ...
    ROUTES.forEach((route, routeIndex) => {
      ...
      route.departures.forEach((departure, departureIndex) => {
        ...
        // Skip departures that are already in the past.
        ...
        flights.push({
          id: `${flightNumber}-${toDateKey(departureAt)}-${pad(hours)}${pad(minutes)}`,
          flightNumber,
          airline: route.airline,
          airlineCode: route.airlineCode,
          originCode: route.origin,
          destinationCode: route.destination,
          departureTime: toLocalIso(departureAt),
          arrivalTime: toLocalIso(arrivalAt),
          durationMinutes: route.durationMinutes,
          aircraft: route.aircraft,
          baseFare: route.baseFare,
          currency: "NGN",
          cabins: AIRCRAFT_LAYOUTS[route.aircraft] ?? AIRCRAFT_LAYOUTS["Boeing 737-800"],
          blockedSeats: [],
          status: "scheduled",
        });
      });
