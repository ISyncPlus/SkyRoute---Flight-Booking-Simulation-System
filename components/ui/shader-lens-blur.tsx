...
/* Palette pulled onto SkyRoute's teal rather than the stock lime/pink, so the
   canvas belongs to the page it sits on. The four stops are the accent, its
   dark-mode lift, the deep brand teal and a cool sky blue. */
...
export function ShaderLensBlur() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      /* Dispose the GPU objects too, not just the renderer. Every theme flip
         and config change re-runs this effect, and a WebGL context that is
         only "disposed" leaks its geometry, material and compiled program. */
      geometry.dispose()
      material.dispose()
      renderer.dispose()
      resizeObserver.unobserve(container)
    }
  }, [config, updateSize, animate, isDark])
  ...
  useEffect(() => {
    if (materialRef.current) {
      ...
      materialRef.current.uniforms.u_isDarkMode.value = isDark
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        /* The canvas is transparent, so this sits on the app's own surface
           token instead of the hard-coded black the source shipped with. */
